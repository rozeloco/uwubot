...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Shan\n' // Nombre
            + 'ORG:Shanduy;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=595991301300:+595 991 301 300\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `@${num.split('@')[0]} bienvenid@ a *${mdata.subject}*, recuerda seguir las reglas para no tener ningún problema.`
                          client.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Bueno, se fue @${num.split('@')[0]}, que la chupe`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Espera un momento, estoy ejecutando el comando.',
				success: 'Listo ✔️',
                                levelon: '❬ ✅ ❭ *Level activado*',
				leveloff: ' ❬ ✅ ❭  *Level desactivado*',
				foto: 'Espera estoy cambiando la foto del grupo.',
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa @r4ssex',
					admin: '[❗] Este comando solo puede ser utilizado por administradores del grupo',
					Badmin: '[❗] Este comando solo se puede usar cuando el bot se convierte en administrador',
                                        pegatina: 'Espera un toque estoy haciendo tu sticker.\n*Recuerda los stickersgif son de 6 segundos*',
					attp: 'Espera un toque estoy haciendo tu texto a sticker \n*Esto puede demorar unos minutos*',
					...
					musica: 'Espera un toque estoy bucando tu canción\n*Recuerda colocar bien el nombre de la cancion o el link del video de youtube*\n\n*Si el comando *play no funciona utiliza el comando *play2*',
					musica2: 'Espera un toque estoy bucando tu canción\n*Recuerda colocar bien el nombre de la cancion o el link del video de youtube*\n\n*Si el comando *play2 no funciona utiliza el comando *play*',
					daftarB: `「NO ESTAS REGISTRADO」\n\nNo tengo tu usuario en mis datos.\n\nPara poder usarme escribe el siguente comando\n\nComando: ${prefix}daftar Nombre\nEjemplo: ${prefix}daftar taladroanal69`,
				}
			}
    			const apakah = ['Si','No']
                        const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
			const botNumber = client.user.jid
			const ownerNumber = ["5959913013004@s.whatsapp.net"] // replace this with your number
...
if (budy.includes("https://wa.me/")){
		if (!isGroup) return
		if (!isAntiWa) return
                if (isGroupAdmins) return reply('Eres un administrador del grupo, así que no te prohibiré el uso de enlaces, que la chupen los miembros')
...
//FUNCION ANTILINK
	...
	if (budy.includes("https://discord.gg/")){
		if (!isGroup) return
...
/******ENTRADA FIN DE FUNCIONES******/
			function addMetadata(packname, author) {	
				if (!packname) packname = '🌴🌴'; if (!author) author = 'ig @r4ssex';	
				author = author.replace(/[^a-zA-Z0-9]/g, '');	
				let name = `${author}_${packname}`
...
/******JUEGOS SHANDUY LA PUTA MADRE NO TE OLVIDES******/					
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'creador':
	       client.sendMessage(from, {displayname: "rosse", vcard: vcard}, MessageType.contact, { quoted: mek})
		client.sendMessage(from, 'q pio queres',MessageType.text, { quoted: mek} )
